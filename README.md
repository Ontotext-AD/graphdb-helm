...
3. Patch all GraphDB worker PVs (we do not need the masters) with `"persistentVolumeReclaimPolicy":"Retain"`:

```bash
-- kubectl patch pv <graphdb-pv-name> -p '{"spec":{"persistentVolumeReclaimPolicy":"Retain"}}'
```

4. Uninstall the GraphDB Helm chart:

```bash
helm uninstall graphdb-ee
```

5. Delete the GraphDB PVCs. If a cluster is used, delete all master and worker PVCs.

```bash
kubectl delete pvc <graphdb-pvc-name>
```

6. Delete the GraphDB master PVs. 

```bash
kubectl delete pv <graphdb-master-pv-name>
```

7. Patch the PVs with `"claimRef":null` so they can go from status Released to Available:

```bash
kubectl patch pv <graphdb-pv-name> -p '{"spec":{"claimRef":null}}'
```

8. Patch the PVs with `claimRef` matching the PVCs that will be generated by the `volumeClaimTemplates`. The PVC names that the Helm chart will generate look like this:
...
**Note**: Do not forget to set the namespace to the one you are going to use for the updated Helm chart.
...
9. Install the 10.0-M3 Helm chart. Remember to edit:
   
- `graphdb.node.resources.limits.memory` and `graphdb.node.resources.requests.memory` to the ones used by the old workers
- `graphdb.nodesCount:` to the number of nodes you want - recommended is 3, 5, 7. If its not equal to the number of workers you had in the old cluster, GraphDB will start replicating.
